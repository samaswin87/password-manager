class UserImportService < AbstractUserImportService
  ...
  def import(import_id, user_id)
    ...
    import.import_data_tables.find_in_batches(batch_size: 100) do |batch|
      batch.each do |import_data|
        ...
        import.mappings.each do |key, field|
          if key == 'gender_id'
            user_hash[key] = find_gender(import_data.dynamic_fields[field]).try(:id)
          else
            user_hash[key] = import_data.dynamic_fields[field]
          end
        end
    ...
    import.update_attribute(:parsed_data, users)
